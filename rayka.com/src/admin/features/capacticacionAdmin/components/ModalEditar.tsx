...
type Props = {
  open: boolean;
  capacitacion: CapacitacionConDetalles | null;
  onClose: () => void;
  onSave: (data: CrearCapacitacionRequest & { id_capacitacion: number }) => Promise<void>;
  loading?: boolean;
...
export default function ModalEditar({ open, capacitacion, onClose, onSave, loading, solicitantes, usuarios, cursos }: Props) {
  ...
  const handleSave = async () => {
    ...
    const data = {
      id_capacitacion: capacitacion.capacitacion.id_capacitacion,
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center">
      ...
      <div className="relative z-61 w-full max-w-2xl mx-4 rounded-xl bg-white shadow-xl border border-slate-200 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 sticky top-0 bg-white z-10">
          <h3 className="text-base font-semibold text-slate-900">Editar Capacitación</h3>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-slate-200 sticky bottom-0 bg-white">
          ...
          >
            {loading ? "Actualizando..." : "Actualizar Capacitación"}
          </button>
        </div>
      </div>
    </div>
